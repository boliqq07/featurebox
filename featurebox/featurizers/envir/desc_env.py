...
def get_strategy2_in_spheres(structure, nn_strategy, cutoff,numerical_tol=None, cutoff_name="cutoff"):
    _ = numerical_tol
    if isinstance(structure, StructureOrMolecule):
        structure = aaa.get_atoms(structure)

    nn_strategy_ = copy.copy(nn_strategy)

    setattr(nn_strategy_, cutoff_name, cutoff)

    result = nn_strategy_.calculate_format(structure)

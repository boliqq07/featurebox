...
class BehlerParrinello:
    ...
    def __init__(self, symmetry_parameters=None, Rc=6.5,
                 derivative=False, stress=False, atom_weighted=False):
...
def calculate_G2(Dij, IDs, atomic_numbers, type_set, Rc, parameters, Gtype):
    ...
    results = np.einsum('ijk,k->ijk', term, Cosine(Dij, Rc))  # n2*n1*layer
...
def calculate_G2Prime(Rij, Ri, i, IDs, atomic_numbers, type_set, Rc, parameters, Gtype):
    ...
    term21 = CosinePrime(R1ij, Rc)  # layer
    _term22 = np.einsum('ij,j->ij', dij, Cosine(R1ij, Rc))  # n1*layer
...
if __name__ == '__main__':
    ...
    for a in [5.0]:  # , 5.4, 5.8]:
        ...
        bp = BehlerParrinello(symmetry, Rc=Rc, derivative=False, stress=True, atom_weighted=False)
        ...
        bp = BehlerParrinello(symmetry, Rc=Rc, derivative=False, stress=True, atom_weighted=True)
        des = bp.calculate(si, system=[14])
        print("G:", des['x'][0])
        print("Sequence", des['seq'][0])

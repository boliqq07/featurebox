...
class PropertyStats(object):
    ...
    def mode(data_lst, weights=None):
        ...
        """
        if weights is None:
            return stats.mode(data_lst)[0]
        else:
            # Find the entry(s) with the largest weight
            data_lst = np.array(data_lst)
            weights = np.array(weights)
            most_freq = np.isclose(weights, max(weights))
...
# class UnionFeature(BaseFeature, ABC):
#
...
#
#     def feature(spath, comp_number=0):
#         """
...
#         return spath.feature_many(number, ignore_errors=True)
#
